...
/** Spring for resize-on-click feel (slower, smoother resize with minimal overshoot). */
const bubbleSizeTransition = {
  type: "spring" as const,
  stiffness: 130,
  damping: 28,
};
const textTransition = { duration: 0.38, ease: "easeInOut" as const };
...
export function VoicePromptWithBubble({
  ...
  return (
    ...
    >
      ...
      {/* Revealed text when focused — fade in/out only; slot has min-height to prevent layout shift when text unmounts */}
      <div className="min-h-[2.5rem] flex flex-col items-center justify-center w-full max-w-md">
      ...
      <div className="w-full max-w-md flex items-center gap-2 rounded-3xl border border-border bg-muted/20 px-3 py-2 focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/20 focus-within:ring-offset-shadow-blue-200 focus-within:ring-offset-2 transition-all">
        <button
          type="button"
        ...
        <Input
          ...
          onFocus={handleFocus}
          onBlur={handleBlur}
          className="flex-1 min-w-0 border-0 bg-transparent shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:shadow-none placeholder:text-muted-foreground text-sm sm:text-base"
